/**
 * Direction:
 * Return a formatted array of sessions with list of classes & students
 * 
 * Expected Result:
 * [
 *  {
 *    session_id: 1,
 *    time: '09:00',
 *    classes: [
 *      {
 *        class_id: 1,
 *        name: 'A',
 *        students: [
 *          { student_id: 1, name: 'Adi' },
 *          { student_id: 1, name: 'Budi' },
 *        ],
 *      },
 *      {
 *        class_id: 2,
 *        name: 'B',
 *        students: [
 *          { student_id: 3, name: 'Bayu' },
 *          { student_id: 4, name: 'Dharma' },
 *        ],
 *      },
 *    ],
 *  },
 *  {
 *    session_id: 2,
 *    time: '10:00',
 *    classes: [
 *      {
 *        class_id: 3,
 *        name: 'C',
 *        students: [
 *          { student_id: 5, name: 'Surya' },
 *          { student_id: 6, name: 'Maha' },
 *        ],
 *      },
 *      {
 *        class_id: 4,
 *        name: 'D',
 *        students: [
 *          { student_id: 7, name: 'Dede' },
 *          { student_id: 8, name: 'Edi' },
 *        ],
 *      },
 *    ],
 *  },
 * ];
 */

const sessions = [
  { session_id: 1, time: '09:00', student: { student_id: 1, name: 'Adi' }, class: { class_id: 1, name: 'A' } },
  { session_id: 2, time: '10:00', student: { student_id: 5, name: 'Surya' }, class: { class_id: 3, name: 'C' } },
  { session_id: 2, time: '10:00', student: { student_id: 8, name: 'Edi' }, class: { class_id: 4, name: 'D' } },
  { session_id: 2, time: '10:00', student: { student_id: 7, name: 'Dede' }, class: { class_id: 4, name: 'D' } },
  { session_id: 1, time: '09:00', student: { student_id: 3, name: 'Bayu' }, class: { class_id: 2, name: 'B' } },
  { session_id: 1, time: '09:00', student: { student_id: 2, name: 'Budi' }, class: { class_id: 1, name: 'A' } },
  { session_id: 1, time: '09:00', student: { student_id: 4, name: 'Dharma' }, class: { class_id: 2, name: 'B' } },
  { session_id: 2, time: '10:00', student: { student_id: 3, name: 'Maha' }, class: { class_id: 3, name: 'C' } },
];


function result(sessions) {
  const finalArray = []
  for(let session of sessions){
      if(finalArray.length == 0){
          finalArray.push(addNewObject(session))
      }
      else{
          const found = finalArray.some(el => el.session_id === session.session_id)
          if(!found){
              finalArray.push(addNewObject(session))
          }else{
              let result = finalArray.filter(x => x.session_id === session.session_id)
              let data = result[0]
              let classExisted = data.classes.find(x => x.class_id === session['class'].class_id)
              if(classExisted != undefined){
                  let studentData = new Object
                  studentData.student_id = session['student'].student_id
                  studentData.name = session['student'].name
                  classExisted.students.push(studentData)
              }
              else{
                  let classData = new Object
                  classData.class_id = session['class'].class_id
                  classData.name = session['class'].name
                  classData.students = []
                  let studentData = new Object
                  studentData.student_id = session['student'].student_id
                  studentData.name = session['student'].name
                  classData.students.push(studentData)
                  data.classes.push(classData)
              }
          }
      }
  }
  return finalArray
}

function addNewObject(session){
  let result = new Object
  result.session_id = session.session_id
  result.time = session.time
  result.classes = []
  let classData = new Object
  classData.class_id = session['class'].class_id
  classData.name = session['class'].name
  classData.students = []
  let studentData = new Object
  studentData.student_id = session['student'].student_id
  studentData.name = session['student'].name
  classData.students.push(studentData)
  result.classes.push(classData)
  return result
}

console.log(result(sessions));
